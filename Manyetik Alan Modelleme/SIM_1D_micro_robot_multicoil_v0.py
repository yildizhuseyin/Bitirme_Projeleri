...
# %% KONTROLCÜ AYARLARI 
PID_controller=PID_controller_1D(dt,y_ref=-0.0*3*a/4) # ♠kontrolcüyü tanımla 
PID_controller.set_parameters(kp=500) # PID parametrelerini düzenle 
kc=175; tc=(0.555-0.515)*10
PID_controller.set_ZNC(kc,tc,Type='PID') # PID parametrelerini düzenle 
PID_controller.set_scale_factor([1.0,1.0,1.0])
...
while running:
    ...
    if SIM.adim % 3==0:   
        ...
        fark=PID_controller.apply(SIM.X[0,0]) # Kontrolcüden uygulanacak akımı hesaplat 
        # fark=PID_controller.apply_with_ref(SIM.X[0,0],yref)
        control_current=-3;
        I_1=kk*(-control_current+fark*1.0)
        I_2=kk*(-control_current-fark*1.0)
        if I_1>3: 
            I_1=3
        elif I_1<-3: 
            I_1=-3
