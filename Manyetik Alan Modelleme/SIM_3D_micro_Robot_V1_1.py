...
bobin2=cylindirical_coil_3D(ekran,a/1,i=+I,z0=-a/1,n=N,Type='2D')

bobin3=cylindirical_coil_3D(ekran,a/1,i=1*I,z0=1*a/1,n=N,alfa=[-90,0,0])
bobin4=cylindirical_coil_3D(ekran,a/1,i=-1*I,z0=1*a/1,n=N,alfa=[+90,0,0])
...
while running:
    say=say+1
    if SIM.adim % 2==0:   
        y_ref=15#*np.sin(2*np.pi*SIM.t)
        z_ref=15#*np.sin(2*np.pi*SIM.t)
        ...
        #i_c_y=3*
        i_1=+40*err_z+50*err_y#255#(-err_z*150+i_c_y)*1
        i_2=40*err_z-50*err_y#(-err_z*150-i_c_y)*0
        I_1=1*i_1*(3/255)
        I_2=1*i_2*(3/255)
        ...
        #i_c_z=3
        i_3=-40*err_y+50*err_z#(-err_y*150+i_c_z)*1
        i_4=-40*err_y-50*err_z#(-err_y*150-i_c_z)*0
        I_3=1*i_3*(3/255)
        I_4=1*i_4*(3/255)
    ...
    data.append([SIM.adim,SIM.get_time(),y_ref,z_ref,SIM.X[0,1]*1000,SIM.X[0,2]*1000])
    ...
    if -40/1000<SIM.X[0,0] and 40/1000>SIM.X[0,0] and-40/1000<SIM.X[0,1] and 40/1000>SIM.X[0,1] and -40/1000<SIM.X[0,2] and 40/1000>SIM.X[0,2]:
        ...
        if SIM.adim % 20==0:  
            # ekran.delate_arrows()
            # ekran.plot_magnetic_field_vectors([-00,00,1],[-40,40,5],[-40,40,10],scale=100.0)
...
plot_list_points(1,211,[(Data[:say,2],Data[:say,3],'-k'),
